describe('모델 그룹 관리', () => {
  ...
  it('모델 그룹 조회', () => {
      cy.contains("모델 그룹 관리").click();
      cy.wait(2 * 1000)
      cy.get('h1').contains("모델 그룹 관리")
  ...
  it('모델 그룹 수정', () => {
    ...
    cy.wait(2 * 1000)
    cy.get('h1').contains("모델 그룹 관리")
    cy.get(':nth-child(1) > .button-bg > .MuiButtonBase-root').click() // 상세보기
    cy.contains("수정").click()  
    cy.get('[name="description"]').invoke('val').then((currentValue) => {
    cy.get('[name="description"]').clear().type(currentValue + '_test_1234');
